...
#include "icp_localization/lidar_undistortion.h"
...
LidarUndistortion::LidarUndistortion() : private_node_("~")
{
    ...
    private_node_.param<bool>("use_odometry", use_odom_, false);
    private_node_.param<bool>("laser_inverse", laser_inverse_, true);
...
bool LidarUndistortion::PruneImuDeque()
{
    ...
    for (int i = 0; i < (int)imu_queue_.size(); i++)
    {
        ...
        angular_x = 0.0;
        angular_y = 0.0;
        double vw = tmp_imu_msg.angular_velocity.z;
        angular_z = vw * 3.141592653 / 180.0;
...
void LidarUndistortion::CorrectLaserScan()
{
    ...
    publish_msg.header = current_laserscan_.header;
    publish_msg.time_increment = current_laserscan_.time_increment;
    publish_msg.scan_time = current_laserscan_.scan_time;
    publish_msg.range_min = current_laserscan_.range_min;
    publish_msg.range_max = current_laserscan_.range_max;

    publish_msg.angle_max = current_laserscan_.angle_max;
    publish_msg.angle_min = current_laserscan_.angle_min;
    publish_msg.angle_increment = current_laserscan_.angle_increment;

    for(int alpha = 0; alpha < scan_count_; ++alpha) {
    ...
    for (int i = 0; i < scan_count_; i++)
    {
        ...
        if(laser_inverse_)
        {
          scan_idx = scan_count_ - 1 - i;
        }
        // 如果是无效点，就跳过
        if (!std::isfinite(current_laserscan_.ranges[scan_idx]) ||
            current_laserscan_.ranges[scan_idx] < current_laserscan_.range_min ||
            current_laserscan_.ranges[scan_idx] > current_laserscan_.range_max)
            continue;

        // 畸变校正后的数据
        PointT &point_tmp = corrected_pointcloud_->points[scan_idx];

        current_point_time = current_scan_time_start_ + i * current_scan_time_increment_;

        // 计算雷达数据的 x y 坐标
        current_point_x = current_laserscan_.ranges[scan_idx] * a_cos_[scan_idx];
        current_point_y = current_laserscan_.ranges[scan_idx] * a_sin_[scan_idx];
...
//            point_tmp.z = 0.0;
        ...
        if(rotZCur<0)
        {
          rotZCur += 2 * 3.14159265;
        }

        int index = (int) rotZCur / current_laserscan_.angle_increment;
        if(index >= 0 && index < scan_count_)
        {
          publish_msg.ranges[index] = std::sqrt(point_tmp.x * point_tmp.x + point_tmp.y * point_tmp.y);
          publish_msg.intensities[index] =  current_laserscan_.intensities[scan_idx];
        }
    }
...
void LidarUndistortion::PublishCorrectedPointCloud()
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr clear_pointcloud_pcl(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointXYZI point_xyzi;

    for(int i=0; i<corrected_pointcloud_->points.size(); i++)
    {
        float px = corrected_pointcloud_->points[i].x;
        float py = corrected_pointcloud_->points[i].y;
        float dist = std::sqrt(px*px+py*py);
        if(dist>0.3)
        {
            clear_pointcloud_pcl->points.push_back(corrected_pointcloud_->points[i]);
        }
    }

    // ROS_INFO("start cluster ...");
    // pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);
    // tree->setInputCloud (corrected_pointcloud_);
    ...
    // ec.setInputCloud (corrected_pointcloud_);
    // ec.extract (cluster_indices);
    // ROS_INFO("complete cluster!");
    ...
    //     {
    //       float px = corrected_pointcloud_->points[*pit].x;
    //       float py = corrected_pointcloud_->points[*pit].y;
    //       float dist = std::sqrt(px*px+py*py);
    //       if(dist>0.3)
    //       {
    //         clear_pointcloud_pcl->points.push_back(corrected_pointcloud_->points[*pit]);
    //       }
    ...
    pcl_conversions::toPCL(current_laserscan_header_, clear_pointcloud_pcl->header);

    // 由于ros中自动做了 pcl::PointCloud<PointT> 到 sensor_msgs/PointCloud2 的数据类型的转换
    // 所以这里直接发布 pcl::PointCloud<PointT> 即可
    corrected_pointcloud_publisher_.publish(clear_pointcloud_pcl);
}
